...
export type AdminViewType = 'dashboard' | 'users' | 'banks' | 'products' | 'loans' | 'analytics' | 'settings' | 'homepage'
...
const navigationCategories: NavCategory[] = [
  {
    ...
    items: [
      ...
      { id: 'analytics', label: 'Analytics', icon: BarChart3, description: 'Reports & metrics' },
    ]
  },
  {
    id: 'management',
    ...
    icon: Layers,
    items: [
      { id: 'users', label: 'Users', icon: Users, description: 'Manage user accounts', badge: 12 },
      { id: 'banks', label: 'Banks', icon: Building2, description: 'Partner institutions' },
      { id: 'products', label: 'Products', icon: Package, description: 'Loan products catalog' },
      { id: 'loans', label: 'All Loans', icon: FileText, description: 'Loan applications', badge: 5 },
    ]
  },
  {
    id: 'content',
    ...
    icon: Monitor,
    items: [
      { id: 'homepage', label: 'Homepage', icon: Monitor, description: 'Manage homepage sections' },
    ]
  },
  {
    id: 'system',
...
export function AdminLayout({ children, user, currentView, onViewChange }: AdminLayoutProps) {
  ...
  const handleNavigation = (view: AdminViewType) => {
    ...
    onViewChange(view)
    router.replace(`/?view=admin&tab=${view}`, { scroll: false })
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-100 via-gray-50 to-gray-100">
            ...
            <div className="mx-4 mt-4 mb-2">
              <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-emerald-500/20 to-teal-500/20 border border-white/10 p-3">
                ...
                <div className="relative flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-xs font-medium text-white">AI-Powered</div>
                    <div className="text-[10px] text-white/60">Smart Analytics</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            {!sidebarCollapsed ? (
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-white/50 truncate capitalize">{user.role.toLowerCase().replace('_', ' ')}</p>
                </div>
                <Button
                  variant="ghost"
                  size="icon"
      ...
      <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-gray-200/50 bg-white/80 backdrop-blur-xl px-4 lg:hidden">
            ...
            <div className="flex h-full flex-col">
              ...
              <div className="border-t border-white/10 p-4">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-white truncate">{user.fullName}</p>
                    <p className="text-xs text-white/50 truncate capitalize">{user.role.toLowerCase().replace('_', ' ')}</p>
                  </div>
                </div>
                <Button
                  variant="ghost"
        ...
        <div className="flex items-center gap-2">
          ...
          <Badge className="bg-gradient-to-r from-emerald-500 to-teal-500 text-white border-0 text-[10px] ml-1">Admin</Badge>
        </div>
