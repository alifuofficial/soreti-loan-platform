...
export type BankerViewType = 'dashboard' | 'pending' | 'approved' | 'rejected' | 'stats' | 'loan' | 'profile'
...
const navigationCategories: NavCategory[] = [
  {
    ...
    icon: PieChart,
    items: [
      { id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard, description: 'Bank overview' },
  ...
  },
  {
    ...
    icon: FileText,
    items: [
      { id: 'pending', label: 'Pending Review', icon: FileText, description: 'Awaiting review' },
      { id: 'approved', label: 'Approved', icon: CheckCircle, description: 'Approved loans' },
      { id: 'rejected', label: 'Rejected', icon: XCircle, description: 'Rejected loans' },
    ]
  },
  {
    id: 'settings',
    ...
    icon: Settings,
    items: [
      { id: 'profile', label: 'Profile Settings', icon: User, description: 'Account & security' },
    ]
  }
]
...
interface NavCategoryProps {
  ...
  currentView: BankerViewType
  onNavigate: (view: BankerViewType) => void
  onClose?: () => void
  collapsed?: boolean
...
interface BankerLayoutProps {
  ...
  currentView: BankerViewType
  onViewChange: (view: BankerViewType) => void
}

export function BankerLayout({ children, user, currentView, onViewChange }: BankerLayoutProps) {
  const router = useRouter()
  const [sidebarOpen, setSidebarOpen] = useState(false)
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false)

  const handleNavigation = (view: BankerViewType) => {
    if (view === currentView) return
    onViewChange(view)
    router.replace(`/?view=banker&tab=${view}`, { scroll: false })
  }
  ...
  const currentPageLabel = navigationCategories
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-100 via-gray-50 to-gray-100">
        ...
        <div className="relative flex h-full flex-col">
          ...
          {!sidebarCollapsed && (
            <div className="mx-4 mt-4 mb-2">
              <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-blue-500/20 to-indigo-500/20 border border-white/10 p-3">
                ...
                <div className="relative flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-xs font-medium text-white">{user.bankName || 'Your Bank'}</div>
                    <div className="text-[10px] text-white/60">Assigned Bank</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            {!sidebarCollapsed ? (
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-white/50 truncate">Banker</p>
                </div>
                <Button
                  variant="ghost"
                  size="icon"
      ...
      <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-gray-200/50 bg-white/80 backdrop-blur-xl px-4 lg:hidden">
            ...
            <div className="flex h-full flex-col">
              ...
              <div className="border-t border-white/10 p-4">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-white truncate">{user.fullName}</p>
                    <p className="text-xs text-white/50 truncate">{user.bankName || 'Banker'}</p>
                  </div>
                </div>
                <Button
                  variant="ghost"
        ...
        <div className="flex items-center gap-2">
          ...
          <Badge className="bg-gradient-to-r from-blue-500 to-indigo-500 text-white border-0 text-[10px] ml-1">Banker</Badge>
        </div>
      ...
      <main className={cn(
        ...
        <header className="hidden lg:flex h-16 items-center justify-between border-b border-gray-200/50 bg-white/80 backdrop-blur-xl px-6 sticky top-0 z-20">
          <div className="flex items-center gap-4">
            <div>
              <h1 className="text-lg font-semibold text-gray-900">{currentPageLabel}</h1>
              <p className="text-xs text-gray-500">{user.bankName || 'Bank Dashboard'}</p>
            </div>
          </div>
